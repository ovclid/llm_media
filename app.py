...
def start():
    ...
    """
    st.markdown(hide_manage_app_style, unsafe_allow_html=True)
    ...
    user_question = st.text_input(
        "ㅇ 온누리상품권 Q&A - RAG(검색 증강 생성) 적용 AI",
    ...
    # Handle the response
    if "error" in result:
        error_msg = f"<span style='color:red;'>{result['error']}</span>"
        if result['error'] == "주소를 인식할 수 없습니다.":
            error_msg = error_msg + " 확인 후 다시 입력해 주시기 바랍니다. 참고로 충북중기청 주소는 <span style='color:blue;'>'중심상업2로 48' </span>입니다."
        else:
            error_msg = error_msg + " 일반적인 내용을 토대로 답변을 원하시면 <span style='color:blue;'>질문 앞에 #를 붙여주세요.</span>"
