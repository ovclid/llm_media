...
def start():
    ...
    with col3:
        ...
        st.image(image_path, width=150, use_column_width=True)  # Adjust width as needed
    ...
    if result.get("pos"):
        ...
        if result["market_in"] == "":
            st.markdown(f"""<span style='color:red; font-weight:bold;'>어느 구역에도 속하지 않는 것으로 추정됩니다.</span>""", unsafe_allow_html=True)
